...
def bifurk_my_1(DT, H, G, L0, L1, fig=None):
    DT = np.array(DT, dtype=float).flatten()
    H = np.array(H, dtype=float).flatten()
    G = np.array(G, dtype=float).flatten()

    if len(DT) < L0 + L1 or len(H) < L0 + L1 or len(G) < L0 + L1:
        logger.error("Недостаточно данных для кумулятивных сумм")
        return

    # Очистка NaN/Inf с предупреждением
    for name, arr in [('DT', DT), ('H', H), ('G', G)]:
        nan_count = np.count_nonzero(~np.isfinite(arr))
        if nan_count > 0:
            logger.warning("Обнаружено %d NaN/Inf значений в %s — заменены на 0", nan_count, name)
    ...
    try:
        ...
        logger.error("Ошибка в кумулятивных суммах: %s", e, exc_info=True)
        raise
